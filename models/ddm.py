...
#         self.high_enhance1 = HFRM(in_channels=3, out_channels=64)
#         self.Unet = DiffusionUNet(config)
#         self.uvit = UVit(config)  # Initialize the U-ViT model
#
#         betas = get_beta_schedule(
#             beta_schedule=config.diffusion.beta_schedule,
#             beta_start=config.diffusion.beta_start,
...
#         n, c, h, w = x_cond.shape
#         seq_next = [-1] + list(seq[:-1])
#         x = self.uvit(x_cond)  # Use U-ViT to generate initial noise
#         xs = [x]
#         for i, j in zip(reversed(seq), reversed(seq_next)):
#             t = (torch.ones(n) * i).to(x.device)
#             next_t = (torch.ones(n) * j).to(x.device)
...
#             c1 = eta * ((1 - at / at_next) * (1 - at_next) / (1 - at)).sqrt()
#             c2 = ((1 - at_next) - c1 ** 2).sqrt()
#             xt_next = at_next.sqrt() * x0_t + c1 * self.uvit(x_cond) + c2 * et  # Use U-ViT for noise here as well
#             xs.append(xt_next.to(x.device))
#
#         return xs[-1]
#
...
#         a = (1 - b).cumprod(dim=0).index_select(0, t).view(-1, 1, 1, 1)
#
#         e = self.uvit(input_LL_LL)  # Use U-ViT to generate noise here
#
#         if self.training:
#             gt_img_norm = data_transform(x[:, 3:, :, :])
#             gt_dwt = dwt(gt_img_norm)
